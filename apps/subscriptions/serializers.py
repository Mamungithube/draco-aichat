...
class SubscriptionStatusLogSerializer(serializers.ModelSerializer):
    class Meta:
...
class StartTrialSerializer(serializers.Serializer):
    ...
    def validate_plan_id(self, value):
        if not SubscriptionPlan.objects.filter(id=value, is_active=True).exists():
            raise serializers.ValidationError("এই প্ল্যান ID টি বৈধ নয়")
        return value
...
class VerifyPaymentSerializer(serializers.Serializer):
    payment_intent = serializers.CharField(max_length=100, required=True)